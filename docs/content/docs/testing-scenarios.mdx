...
HEIGHT=$(curl -s -X POST --data '{
  ...
  http://localhost:9630/v1/chain/P | jq -r '.result.height')
...
X_CHAIN_URL="http://localhost:9630/v1/chain/X"
C_CHAIN_URL="http://localhost:9630/v1/chain/C/lux"
...
curl -X POST --data '{
  ...
  http://localhost:9630/v1/chain/${BLOCKCHAIN_ID}
```
...
for i in {1..100}; do
  ...
  curl -X POST --data '{
    ...
    http://localhost:9630/v1/chain/P
